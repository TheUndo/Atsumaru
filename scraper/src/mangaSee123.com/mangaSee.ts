...
export async function scrapeInfoPage(
    ...
    const title = $(".BoxBody .list-group-item h1").text() ?? "Untitled",
        ...
        genres =
            items.get("Genre(s):")?.map((genre) => genre.toLowerCase()) ?? [],
        ...
        officiallyTranslated =
            items.get("Official Translation:")?.[0] ?? "unknown",
    ...
    function normalizeChapter(
        ...
        const name = normalizeMangaSeeChapterName(rawChapter.Chapter);
        ...
        await (async () => {
            ...
            for (const [i, chapter] of chapters.entries()) {
                ...
                const { data, error } = await aw<string>(
                    goLangify(
                        axios(`https://mangasee123.com${chapter.chapterURL}`)
                    )
                );
                if (error || !data) {
                    void log(
                        "Error while scarping page from: ",
                        slug,
                        "error: ",
                        error
                    );
...
function extractChapters(html: string) {
    ...
    const endChapters = startChapters.slice(
        0,
        startChapters.indexOf("}];") + 2
    );
 ...
 */
function normalizeMangaSeeChapterName(chapterName: string) {
    const left = parseInt(chapterName.slice(1, -1));
    const right = chapterName[chapterName.length - 1];
    return 0 === +right ? left.toString() : `${left}.${right}`;
}
